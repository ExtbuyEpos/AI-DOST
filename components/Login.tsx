...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const speakWelcome = () => {
    if ('speechSynthesis' in window) {
      const utterance = new SpeechSynthesisUtterance("Welcome back Sir. Initializing fast uplink.");
      utterance.rate = 1.1;
      utterance.pitch = 1.0;
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setStatusText('RAPID_BYPASS_INITIATED...');
    
    let progress = 0;
    const interval = setInterval(() => {
      progress += Math.random() * 25; // Massive speed boost
      if (progress >= 100) {
        progress = 100;
        clearInterval(interval);
        setStatusText('IDENTITY_SYNCED.');
        ...
        }, 300); // Near instant transition
      }
      setScanProgress(progress);
      if (progress > 50) setStatusText('ESTABLISHING_L7_TUNNEL...');
    }, 40); // Fast cycle
  };
      ...
      <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black overflow-hidden cursor-pointer" onClick={handleStartInitialization}>
        ...
        <div className="text-center space-y-10 animate-pulse">
           ...
           <div>
              ...
              <p className="text-[10px] orbitron text-slate-500 mt-4 tracking-[0.5em]">CLICK_FOR_FAST_INIT</p>
           </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#010409] overflow-hidden">
      ...
      <div className="relative w-full max-w-7xl h-[85vh] flex flex-col lg:flex-row items-center gap-10 p-10 animate-in fade-in zoom-in duration-700">
        ...
        <div className="hidden lg:flex flex-1 relative items-center justify-center h-full">
           ...
           <div className="absolute bottom-10 left-0 space-y-4">
              <div className="flex items-center gap-4 bg-black/40 px-6 py-3 rounded-2xl border border-cyan-500/20 backdrop-blur-xl">
                 ...
                 <span className="text-[11px] orbitron font-black text-white tracking-widest uppercase">FAST_ACCESS_OS</span>
              </div>
           </div>
        </div>
        ...
        <div className="w-full max-w-lg flex flex-col gap-10">
          <div className="text-center lg:text-left space-y-2">
            ...
            <p className="text-[11px] orbitron text-cyan-800 font-bold uppercase tracking-[0.5em]">Fast_Uplink_v6.0</p>
          </div>
          ...
          <div className="hud-glass p-12 rounded-[4rem] border-cyan-500/20 shadow-[0_0_120px_rgba(0,0,0,0.8)] relative overflow-hidden group">
            ...
            {isAuthenticating ? (
              <div className="py-14 flex flex-col items-center gap-12">
                 <div className="relative w-56 h-56 flex items-center justify-center">
                    ...
                    <div className="flex flex-col items-center">
                       ...
                       <span className="text-[8px] orbitron text-cyan-600 font-bold tracking-widest mt-2 uppercase">VERIFYING...</span>
                    </div>
                 </div>
              ...
              <form onSubmit={handleLogin} className="flex flex-col gap-8">
                <div className="space-y-6">
                  <div className="space-y-3">
                    <input 
                      ...
                      required
                      placeholder="USER_ID"
                      className="w-full bg-black/60 border-2 border-white/5 rounded-[1.5rem] px-8 py-5 text-white font-mono text-sm outline-none focus:border-cyan-500/40 transition-all"
                    />
                  </div>
                  <div className="space-y-3">
                    <input 
                ...
                <button 
                  type="submit"
                  className="w-full py-6 bg-cyan-600 hover:bg-cyan-500 text-white orbitron font-black text-xs rounded-3xl shadow-[0_25px_50px_rgba(6,182,212,0.25)] transition-all hover:scale-[1.03] active:scale-95 uppercase tracking-[0.4em] flex items-center justify-center gap-4"
                >
                  INITIALIZE_MASTER_OS
                </button>
              </form>
            )}
          </div>
