...
export const ProblemSolver: React.FC<ProblemSolverProps> = ({ isOpen, onClose, problems }) => {
  ...
  return (
    <div className="fixed inset-0 z-[170] flex items-center justify-center p-4 md:p-10 pointer-events-none">
      ...
      <div className="relative w-full max-w-5xl h-[80vh] bg-[#050a0a] border border-emerald-500/30 rounded-[3.5rem] overflow-hidden shadow-[0_0_150px_rgba(16,185,129,0.15)] pointer-events-auto flex flex-col animate-in zoom-in duration-300">
        ...
        <div className="h-20 bg-black/60 border-b border-emerald-500/20 flex items-center justify-between px-10">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
               <span className="text-[16px] orbitron font-black text-emerald-400 tracking-[0.3em] uppercase">Fast_Mission_Hub</span>
               <span className="text-[8px] orbitron text-emerald-900 font-black uppercase">Instant_Execution_v6.0</span>
            </div>
          </div>
        ...
        <div className="flex-1 p-12 overflow-y-auto premium-scroll grid grid-cols-1 gap-8">
           {problems.length === 0 ? (
             <div className="h-full flex flex-col items-center justify-center opacity-30 py-20 grayscale">
                <div className="w-32 h-32 border-2 border-emerald-900 rounded-full flex items-center justify-center">
                ...
                <span className="text-xl orbitron font-black text-emerald-900 tracking-[1em] mt-10 uppercase">FAST_LINK_ACTIVE</span>
             </div>
           ) : (
             problems.map(prob => (
               <div key={prob.id} className="hud-glass p-10 rounded-[3rem] border-emerald-500/20 bg-emerald-500/5 animate-in slide-in-from-bottom-4 duration-300 flex flex-col md:flex-row gap-10 group">
                  <div className="flex-1 space-y-6">
                     <div className="flex justify-between items-center">
                        <span className="px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-[10px] orbitron font-black text-emerald-400 tracking-widest uppercase">FAST_ID: {prob.id}</span>
                        <div className="flex items-center gap-3">
                           <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                           <span className="text-[10px] orbitron font-black text-emerald-600 uppercase tracking-widest">RESOLVED</span>
                        </div>
                     </div>
                        ...
                        <div className="flex flex-col gap-2">
                           {[1, 2].map(i => (
                             <div key={i} className="flex items-center gap-4 text-xs font-mono text-emerald-100/40">
                                ...
                                <span>Fast Agent Alpha-0{i}: Solution Mapped. [OK]</span>
                             </div>
                           ))}
                        </div>
                     </div>
                  ...
                  <div className="w-full md:w-64 flex flex-col justify-between p-6 bg-black/40 rounded-[2rem] border border-emerald-500/10">
                     <div className="text-center space-y-2">
                        <span className="text-[8px] orbitron text-emerald-800 font-black uppercase">Efficiency</span>
                        <div className="text-4xl font-mono text-emerald-400 font-black">100%</div>
                     </div>
                     <button className="w-full py-4 bg-emerald-600 text-white orbitron font-black text-[10px] rounded-2xl transition-all uppercase tracking-widest">VIEW_FULL_INTEL</button>
                  </div>
               </div>
             ))
           )}
